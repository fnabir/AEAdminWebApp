...
export function useFileData(fileYear: number, fileNo: number) {
  const [fileInfoData, fileInfoLoading, fileInfoError] = useObject(
    getDatabaseReference(`files/info/${fileYear}/${fileNo}`),
  );
  const [fileDetailsData, fileDetailsLoading, fileDetailsError] = useObject(
  ...
  const [importerData, importerLoading, importerError] = useObject(
    shouldFetchImporter
      ? getDatabaseReference(`info/importer/${fileInfo.importer}`)
      : null,
  );
  const importerTransactionInfo = useObject(
    shouldFetchImporter
      ? getDatabaseReference(
          `transaction/importer/${fileInfo.importer}/bill/${fileYear}-${fileNo}`,
        )
      : null,
  )[0]?.val();
  ...
  const fileName = useMemo(
    () => generateFileCode(fileNo, fileYear, fileInfo?.type),
    [fileNo, fileYear, fileInfo?.type],
  );
  ...
  const dutyData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/duty`),
  )[0];
  const dutyObject = useObject(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/duty`),
  )[0]?.val();
  const staffExpenseData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/staff`),
  )[0];
  const portExpenseData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/port`),
  )[0];
  const customExpenseData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/custom`),
  )[0];
  const otherExpenseData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/other`),
  )[0];
  const deliveryExpenseData = useList(
    getDatabaseReference(`files/expense/${fileYear}/${fileNo}/delivery`),
  )[0];
